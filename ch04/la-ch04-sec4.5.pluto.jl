...
# ╔═╡ 25809a20-fe53-11ec-183f-fb66d7d7e1ef
begin
...
# ╔═╡ ae49784d-641b-4446-b02a-6746e70b1823
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ 5f07d32b-28d1-4bf6-bebd-24155471e512
md"""
# ch04 sec4.5 矩阵的逆

!!! outcomes

    - A. 判断一个矩阵的逆是否存在, 如果存在, 计算逆矩阵

    - B. 用矩阵代数解线性方程组

    - C. 证明矩阵逆的性质

    - D. 判断一个矩阵是否有左逆, 右逆,或者是另一个矩阵的逆矩阵
"""

# ╔═╡ 9924ca18-fba8-47eb-95ad-06a9a361b1b1
md"""
## 4.5.1 矩阵逆的定义和逆矩阵的唯一性


在学习微积分时,定义的函数三要素为 一个定义域, 一个值域, 以及一套规则, 如果定义域的输入与值域的输出一一对应, 那么会存在一套逆向的规则, 把值域作为输入, 通过逆规则可以在定义域中找到唯一的一个输出值. 

从函数定义三要素看矩阵-向量乘法, 以及矩阵-矩阵乘法, 都可以看做函数关系的实例. 输入为一个向量或者一个矩阵, 通过一套规则映射为一个新的向量或者矩阵.  如果输入与输出一一对应则存在唯一的反向规则. 

如果规则是一个矩阵, 逆向的规则也是一个矩阵. 这就是矩阵的逆, 或者叫做逆矩阵
"""

# ╔═╡ 3736eac3-a8a6-48fd-8679-00a84d0e13a1
md"""
!!! definition

    矩阵的逆

    给定矩阵$A,B$, 都是$n \times n$  矩阵.如果有如下关系:

    $BA=I 和 AB=I$

    我们就说$B$  是 $A$ 的逆矩阵. 定义为 $B=A^{-1}$

    如果一个矩阵有逆矩阵存在, 就可以说这个矩阵是可逆的. 
    矩阵和逆矩阵是一一对应的, 所以一个矩阵只有位唯一的逆矩阵

"""

# ╔═╡ 704defd0-cd66-4fa3-a232-f270abac445f
md"""
!!! example 
...
# ╔═╡ dc9f5ef7-70ce-499f-908a-3ee19acf409d
begin
	A=[1 1; 1 2]
	latexify(A)
end

# ╔═╡ f5d1c051-b6bb-4ad7-bb4f-d7de486529ed
begin
	B=[2 -1; -1 1]
	latexify(B)
end

# ╔═╡ 8d5a32ab-bc2a-4b36-a6c3-3cf9aa04c60b
begin
	latexify(A*B)   #A*B=>I 
end

# ╔═╡ 078837fd-8c43-4ad3-a46b-b4c83ba38bc0
begin
	latexify(B*A)   #B*A=>I 
end

# ╔═╡ c2b15dc2-8a9a-489b-9cc6-d013bc685e2e
md"""
基于定义 可以得出 $B$ 为  $A$ 的逆
"""

# ╔═╡ 58d49d48-f92a-4a63-b9f4-8d8dcef5552f
md"""
## 4.5.2 计算矩阵的逆
...
# ╔═╡ acce121e-7d60-4a4b-920a-d444f7e3e9b2
begin
...
# ╔═╡ cb21b518-4075-4232-8986-384635758e0a
md"""
  根据定义我们需要寻找一个矩阵$C^{-1}$, 使得 $C\cdot C^{-1}=I$

  逆矩阵表示如下:
"""

# ╔═╡ 4544a36f-f68d-4a54-82f3-b7184a997ec0
begin
	@variables x y z w
	Inverse=[x z ; y w]
...
# ╔═╡ 9ef2ffdd-9ab7-402f-b998-c56c545eab78
begin
	C*Inverse
end

# ╔═╡ cbd17ed1-76aa-4c29-af4f-9e919f864f0b
md"""
依据上面代码可以列出两个线性方程组
...
# ╔═╡ 5ebcbd07-871d-4073-8db0-d4e3a4758a63
begin
    r1=[1 -2  1 0]
	r2=[2 -3  0 1 ]
	am=[r1;r2]
	...
	#下面执行行操作,得到简化阶梯型矩阵,从而得到方程组的解

end

# ╔═╡ 862dd077-928e-4451-888b-14f0123bacdb
begin
	# 第二行减去两倍的第一行
     r21=r2-2*r1
	 am1=[r1;r21 ]
...
# ╔═╡ 522db797-1bfc-42b8-89c4-7729864e4bb2
begin
	# 第一行加上两倍的第二行
     r12=r1+2*r21
	 am2=[r12;r21]
...
# ╔═╡ f83887b3-31b4-47c8-8a8f-f81bafef650a
md"""
经过行变换的增广矩阵, 现在左边两列是Identity矩阵, 右边两列是两个线性方程组的解, 也就是,$x,y,z,w$
"""

# ╔═╡ 79b2f60c-3a88-48e0-a873-72c92bcd5625
begin
    
	CI=am2[:,3:4]   #  获取第三列和第四列 , [x z;y w]
	latexify(CI)
end

# ╔═╡ 7ed6d4d0-962c-48a0-bd1a-4b09acdccb05
let
	#根据公式验证一下 A*A^-1=I
...
# ╔═╡ aeccd2a0-59f9-4f22-b5f0-47e561039d10
md"""

以上同时解两个线性方程组也就是求矩阵逆的过程 

列出线性方程组的增广矩阵:

$[A|I]$

如果矩阵$A$ 的逆存在, 那么经过行变换可以得到:

$[I|A^{-1}]$

如果增广矩阵的简化阶梯型得到矩阵的秩小于$n$, 则矩阵没有逆. 


矩阵的逆和矩阵的秩, 主元数, 线性无关 的概念内在联系紧密. 这是线性代数学习任务中需要建立的联系之一



求解构造的线性方程组并不是矩阵逆概念的部分, 这部分应该独立出来, 形成独立的方法, 这部分留给计算机去做. 

在 julia 中直接用`inv` 方法求解矩阵逆

例如上面的例子

"""

# ╔═╡ 75680766-ec6b-430a-8b14-46bf03bd3d22
let
	CI=inv(C)
	latexify(CI)
end

# ╔═╡ bcc2c712-ec53-49e2-8c41-b94c688875e3
let
	M=[1 1 ; 2 2]  
	#inv(M)   # 矩阵如果没有逆, 则会提示错误
end

# ╔═╡ 2f80f9cd-8fd2-4854-abbf-b42fe9f4b968
md"""
## 4.5.3 利用矩阵的逆解线性方程组
...
# ╔═╡ ea3610a4-550c-4b5e-b6d3-fb4979ae5ebf
begin
...
# ╔═╡ d6368d41-f67a-4172-beff-36ea1ccb8247
begin
	m4I=inv(m4)
	latexify(m4I)
...
# ╔═╡ f892a4b0-3b5e-4ed0-a8df-fc1de0d70443
begin
...
# ╔═╡ 875466f4-46f5-49ae-9dc6-d28468472dbd
md"""
```julia
sol=m4I*b
```

看起来是一个函数的样式, 我们直接写成函数的的方法
"""

# ╔═╡ 2539a18b-d592-4373-87b9-d8c914a2f6c7
begin
	function equationsystem(matrix)  
	  inverse=inv(matrix)
	  return (b)-> inverse*b
    end

    getx=equationsystem(m4)

    x1=getx(b)
    latexify(x1)
end

# ╔═╡ 4b38548a-df75-4217-8ad3-61b0d0e554ee
md"""
正向的矩阵方程里

$Ax=b$ 

当我们输入一个向量$x$ 映射为一个向量$b$
...
# ╔═╡ 9cc30f6c-8a53-4c12-a4be-40e28bbab778
begin
 # 定义一个新向量 b,得到一个新的 x 向量
  b2=[0;1;3]
  x2=getx(b2)
  latexify(x2)
end

# ╔═╡ c1e68aa8-2ad2-433c-972c-41eec65acef8
md"""
## 4.5.4 矩阵逆的属性


!!! definition
    假设 $A,B$ 为 $n \times n$ 矩阵, $I$ 为  $n \times n$ 全等矩阵, 下面的结论成立:

    1.  $I$ 的逆矩阵为$I^{-1}=I$

    2.  $A,B$ 如果是可逆矩阵, 则$AB$ 也是可逆矩阵, 并且有$(AB)^{-1}=B^{-1}A^{-1}$

    3.  如果$A$ 是可逆的, $A^{-1}$ 也是可逆的, 并且有$(A^{-1})^{-1}=A$

    4.  如果$A$ 是可逆的,那么$(A^k)^{-1}=(A^{-1})^k$

    5.   如果$A$ 是可逆的, $p$ 是非$0$ 标量, 那么$(pA)^{-1}=\frac{1}{p}A^{-1}$
"""

# ╔═╡ 6b997cd7-dbb4-443e-af5b-6fdebc0ee10a
md"""
## 4.5.5 左逆和右逆

当矩阵的行列数不等时, 矩阵必然有现象相关列(这一点后面会涉及到), 如果矩阵列线性相关, 则一定没有逆矩阵. 


但是存在左逆或者右逆, 下面看看左逆和右逆的定义
...
!!! definition
    ...
    设$A$ 为 $m \times n$ 矩阵, $B$ 为  $ n\times m$ 矩阵,如果有
...
# ╔═╡ 85139816-8e05-439d-bbd4-12a8c7f1a961
begin
	E=[1 0 0; 0 1 0]
	latexify(E)
end

# ╔═╡ e612e95d-cfbf-4bab-a616-eb2fb23dc2f9
begin
	F=[1 0 ; 0 1 ;0 0]
	latexify(F)
end

# ╔═╡ 6acf8760-4fc0-444e-919e-6be75e658fdf
let
	res1=E*F
	latexify(res1)
end

# ╔═╡ 98736118-7ea7-4b6b-96f9-96a6af806bbe
md"""

根据定义 $E$ 矩阵是右可逆的, 右逆矩阵为$F$
"""

# ╔═╡ c46c8cb4-f7ac-46e4-bfa9-06aba0541653
let
	res2=F*E
	latexify(res2)
end

# ╔═╡ f687690e-5563-431c-ae3e-14cb9fd5e662
md"""
 矩阵$E$ 是左乘$F$ 没有得到 $I$ 矩阵(对角有$0$ 元素), 所以矩阵$F$ 不是$E$ 的左逆
"""

# ╔═╡ ae0db221-6fc4-4f82-85b1-7e59c8ff423b
md"""
!!! theorem
    设$A$ 为 $m \times n$ 矩阵,

    - 如果$A$ 是左逆的, 则有 $m \geq n$
    - 如果$A$ 是右逆的, 则有 $m \leq n$
    - 如果$A$ 有逆矩阵, 则有 $m=n$


方阵是矩阵可逆的必要条件, 但不是充分条件.如果矩阵满秩(n=rank),则矩阵一定可逆
