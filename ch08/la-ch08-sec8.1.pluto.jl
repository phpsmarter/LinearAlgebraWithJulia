...
# ╔═╡ 941f1b25-2153-487e-a930-167969dbdd69
begin
        using PlutoUI, Plots,HypertextLiteral,LaTeXStrings,Symbolics,LinearAlgebra,Latexify,PolynomialFactors,RowEchelon,Polynomials
        gr()
        theme(:bright)
...
# ╔═╡ bc3bc5c6-234c-4b33-a594-ec30730ea999
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ 711acb84-6ebb-4c3e-9413-7689a63b81ec
md"# ch07 sec7.1 特征向量和特征值

!!! outcomes

    - A. 判断一个向量是否是一个矩阵的特征向量
    - B. 给定一个特征向量, 求对应该向量的特征值
    - C. 给定一个特征值, 求对应该向量的特征向量
    - D. 求一组特征向量作为基的特征空间


数学对象和现实世界客观存在的物体一样有各种不同的性质, 可以看到内在的性质, 比如对角矩阵的性质, 对称矩阵的性质, 恒等矩阵的性质.  矩阵的性质也可以通过它的行为来观察, 特征向量(eigenvector)和特征值(eigenvalue) 就是某些矩阵表现出来的行为, 我们不需要深入到矩阵内部去了解这些性质. 

如果这些外部行为足够多, 可以通过这些外显的性质刻画出矩阵完整的内部性质. 这就是特征值和特征向量的意义.


对于一个小型的矩阵,观察矩阵内部的结构, 或者做简单的化简就可以得到信息, 对于大型的矩阵, 更好的方法是从外部观察它的性质和行为. 


由于计算机的辅助, 通过软件和编程语言获得 特征值和特征向量能简化为一个命令或者一行代码, 所以不要在计算上花费太多时间. 理解其中的意义和性质是最重要的. 


"

# ╔═╡ 78a4c321-6f5a-4dbb-9398-bb718bb73fa7
md"""
## 特征值和特征向量的定义
...
语言表述为: 通过矩阵将$R^n$ 空间的向量映射为$R^m$ 的向量.

如果有 $m=n=rank$, 那么在有些特殊条件下会有特殊情况出现:

$Av=λv,  \ v \ \in R^n,\lambda \in R$


语言描述为: 矩阵$A$ 将$R^n$ 空间的向量映射为$R^m$ 空间的向量$\lambda v$ $\lambda$为标量, 其中$m=n, \lambda \in R$


这里需要考虑函数的概念, $v$ 是在定义域, $\lambda v$ 是在值域, 尽管两种维度一样, 标示的性质不同.不失一般性, 我们首先看看$n=1$ 的情况.

"""

# ╔═╡ 50d83471-6d12-41da-b032-e95e9f334c21
let
	@variables A, v, λ
	# 定义一个一维的矩阵, 例如超市苹果单位为 4元/斤,映射规则
    A=[4]
	# 定义重量向量 v, 购买 10斤
	v=[10]
	λ=4
...
# ╔═╡ a9d7aeb1-e6d8-4124-836d-fb7a44203f00
md"""
在上面的模型中, 通过一个价格和质量的关系将一个一维的重量向量映射为一个一维的价格向量. 这是一维的, 我们接着看二维的情况. 

在超市中购买两种水果, 苹果和梨, 首先构造表示映射关系的矩阵, 不考虑线性相关和无关问题, 所以直接构造一个对角矩阵
"""

# ╔═╡ a3700f6d-51eb-4f9a-9d34-66bea6b73b94
begin
	@variables A, v, λ,m
	A=diagm([4,4])
	latexify(A)
    # 第一列代表苹果的单价向量, 第二列代表梨的单价向量.  
	# 当我们输入一个两种水果的重量向量时, 希望映射为购物小票中的一列付款向量. 
    # 首先构造重量向量
    
	
end

# ╔═╡ c9d788a0-00a4-422e-b85a-92971bd67ae1
let
	# 重量向量 v
...
# ╔═╡ 384ba26f-48f2-480a-aff6-aef65781bd04
let
	v=[3,2]
	latexify(A*v)
end

# ╔═╡ 914be853-75e8-4bbe-a159-a08d5a964aea
let
	λ,v=4,[3,2]
	latexify(λ*v)
end

# ╔═╡ abb6bf08-1ee9-41ca-8fc1-0678eccabab6
md"""
从以上计算可以看到, 通过矩阵向量乘法获得结果和标量$λ$ 乘以一个向量的结果 一样
"""

# ╔═╡ 8e246e20-6835-4146-bf65-0ab39631d053
md"""

!!! definition
    给定一个$m\times n 矩阵 A, m=n \in R$,假设向量 $v \in R^n$,为非$0$ 向量, 如果有标量$λ$
    , 满足以下等式:

    $Av=λv$

     $v$ 就称为 特征向量, $λ$ 称为对应于$v$ 的特征值
"""

# ╔═╡ 1b2483e2-a682-419d-9630-8c35dc030804
md"""
!!! example
...
# ╔═╡ 96dd99bc-e8be-4a9b-aaca-70a74f7c3f85
begin
	A1=[3 -2 2 ; 1 2 1 ;0 2 1]

	latexify(A1)
end

# ╔═╡ 2f6dcc4f-331a-4b19-b599-4d37a854c924
md"""
判断以下向量是否是矩阵 A1 的特征向量, 如果是, 对应的特征值
"""

# ╔═╡ c3d8a50b-5551-48c8-a558-29bb6dda9259
begin
	@variables  v1,v2, v3, v4

	v1,v2,v3,v4=[2, -1, -2],[0,1,1],[1,1,1],[0,0,0]
	[latexify(v1),latexify(v2),latexify(v3),latexify(v4)]
end

# ╔═╡ 68d736bd-2a2e-460a-9d35-87b42ce3ede8
begin
	[latexify(A1*v1),latexify(A1*v2),latexify(A1*v3),latexify(A1*v4)]
end

# ╔═╡ c74c2017-6aa0-4afc-ad00-2774c65f2e3d
begin
	latexify(A1*v1~2*v1)
	# 所以 v1 为特征向量, 特征值为 2
end

# ╔═╡ f4e5cdf3-3599-456e-9d2b-72067195cb1b
begin
	latexify(A1*v2~3*v2)
	# 所以 v2 为特征向量, 特征值为 3
end

# ╔═╡ c809c4f4-a253-4d69-8670-7ecee0e43272
begin
	latexify(A1*v4~3*v4)
	# v4 满足特征向量的定义, 但是对计算没有意义, 所以一般不考虑为特征向量
end

# ╔═╡ 542e434b-a24a-4be8-8942-e3ca2328e207
begin
	
	 #  v3 不符合定义, 所以不是特征向量
end

# ╔═╡ 2562eafe-99b9-4ed3-b018-d5ce8bd72600
md"""
!!! example
    example 2
    求下面矩阵, 特征值为 2时的特征向量
...
# ╔═╡ aac4c74a-a4cd-41b7-944e-fa2da0ac53f5
let
	@variables A2
	A2=[2 0 0 ; -1 3 1 ;2 -2 0]

	latexify(A2)
end

# ╔═╡ 3c6d367f-ecfc-40af-b8db-679a3b0d0035
md"""
根据特征向量定义有:
"""

# ╔═╡ 184fc52e-4597-4284-963a-77ec2f34d8ec
let
	@variables A2,v2,λ2,I
	A2=[2 0 0 ; -1 3 1 ;2 -2 0]
	I=diagm([1,1,1])
	λ2=2

	# 因为有A2*v2=λ2*v2,变形为  (A-2I)v=λv
	# 矩阵 (A-2I) 的阶梯型为:
    latexify(rref(A2-2I))

	#只有一个主元列, 有两个自由变量,所以齐次线性方程 的解集为:

	
end

# ╔═╡ 2301d92b-c657-41e7-9678-be7da2347155
md"""
$v=s\begin{bmatrix}
1\\ 
1\\ 
...
由这个解集可以知道对应特征值的特征向量是由两个向量张成的线性空间, $0$ 向量也符合条件. 所以特征向量是一个$R^3$ 的子空间,这就是特征空间的一个实例,下面定义特征空间
"""

# ╔═╡ 870088de-bf49-490d-b68f-7cb9569eb249
md"""
!!! definition

    特征空间的定义

      给定一个$m\times n 矩阵 A, m=n \in R$, 如果有标量$λ$ 为$A$ 的特征值, 对应于$λ$的特征空间为:

    $E_{\lambda } =\left \{ v |Av=λv \right \}$
...
# ╔═╡ 365a5bc2-69f7-4e92-b074-684687f51106
