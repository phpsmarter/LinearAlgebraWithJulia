...
# ╔═╡ 9611530e-fc3e-11ec-1145-6be9934e692e
begin
...
# ╔═╡ d694b8cf-1b51-4390-9d42-ec60db5acd9a
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ 1f7e3ce1-08d2-4e36-bef6-05d51b7e92a7
md"""
# ch02 sec2.6 点积

!!! outcomes
    - A. 计算向量的点积
    - B. 借助点积的性质, 包括柯西不等式, 三角不等式来证明向量相等或者不等
    - C. 借助点积判断两个向量是否正交
    - D. 计算一个向量在另一个向量上的投影
    - E. 把向量分解为正交的组分
...
向量间的乘法有两种, 一种是点积, 一种是叉积, 这一小节先来看看点积
"""

# ╔═╡ d81d8ad4-1459-4d6d-aca1-c8026ea67484
md"""
...
!!! definition
    ...
    给定$R^n$ 中两个向量u:$\begin{bmatrix} u_1\\ \vdots \\u_n \end{bmatrix}$ 和 $v:\begin{bmatrix} v_1\\ \vdots \\v_n \end{bmatrix}$,点积定义为:

    $u \cdot v= u_1v_1+\cdots+u_nv_n$

两个向量对应位置的分量乘积, 然后求和. 结果会得到一个标量. 

"""

# ╔═╡ 61be1e15-3aa6-4307-8640-7562e9177da3
md"""
!!! example

    example 1

    计算 $u=[1,2,0,-1]'$ 和$v=[0,1,2,3]'$ 的点积

$u\cdot v=(1)(0)+(2)(1)+(0)(2)+(-1)(3)$ 
$=0+2+0-3$
$=-1$
"""
...
# ╔═╡ 2bd2cc44-fdc2-46ec-9d82-7b96f8b513f8
dot([1,2,0,-1],[0,1,2,3])  # julia  自带点积计算方法

# ╔═╡ 682cc373-fd33-4fdd-9d08-c2ce91ea0fcd
md"""
## 点积的性质
...
!!! props
    ...
    -  $u \cdot v=v \cdot u$
    -  $u\cdot u \geq 0$, $u\cdot u=0$ 只有在$u=0$ 时满足条件
    -  $(ku+lv)\cdot w=k(u\cdot w)+l(v\cdot w)$
    -  $u(kv+lw)=k(u\cdot v)+l(u\cdot w)$
    -  $u\cdot u=||u||^2$
"""

# ╔═╡ ef36fa81-94bb-4509-b9ef-7c30ab11f694
md"""

!!! example  
    example 2  用点积计算向量的长度

    给定向量 $u=\begin{bmatrix}2\\ 1\\4 \\2 \end{bmatrix}$,使用点积计算长度
...
$u\cdot u= 2^2+1^2+4^2+2^2=25$
所以 $|u|=5$
"""

# ╔═╡ 7ff5b163-7de6-456f-b9a0-cce3a5bc9740
let
	u=[2,1,4,2]
	sqrt(dot(u,u))
end

# ╔═╡ 0f2c0de3-a61d-485b-8e6b-304f9ba8a2b0
md"""
## 柯西不等式和三角不等式
...
!!! definition
    ...
    给定两个向量:$u$ 和$v$  有下面不等式成立:

    $|u\cdot v| \leq ||u|| \ ||v||$

    如果两个向量方向相同, 则等式成立.

    因此, 柯西不等式为判断两个向量的方向提供了数值依据
...
!!! definition
    ...
    两个向量与向量的和组成三角形, 两边之和大于等于第三边, 当两个向量方向相同时,等式成立
"""

# ╔═╡ 3edf8de0-e645-4d9a-9ba5-4ea18beb6f7a
md"""
## 点积的几何特性

两个向量的夹角定义为$\theta$,范围在$0 \leq \theta \leq \pi$
...
# ╔═╡ 6f63ffc2-638f-4a95-ba0b-f9cdf077a931
md"""

!!! example
    example 3

    求向量$\begin{bmatrix}2\\2  \end{bmatrix}$,$\begin{bmatrix}0\\ 3\end{bmatrix}$ 之间的夹角. 
...
# ╔═╡ eee7af22-4e43-4fd3-8169-f22e6b5e4541
md"""
## 正交向量

正交的意思是两个向量的夹角为$90^{\circ}$ . 在直角坐标系中, 我们一直都在使用正交向量, 二维坐标系下的分别与$x \ , y$ 轴方向相同的向量之间都是正交的. 在几何里称为垂直. 

在线性代数中约定任何向量与零向量都正交. 这个约定没有几何意义, 只是为了理论完整性需要所设定. 
...
!!! defintion
    ...
    我们就说 向量 $u \perp v$, 表示 $u$ 和$v$ 正交

   因为有$cos90^{\circ}=0$, 所以余弦值为 0, 两个向量的正交
    
"""

# ╔═╡ 860f6eab-449f-478e-a4ab-3148741c8f85
md"""
...
从下面计算结果可以知道两个向量夹角为$90^{\circ}$, 是正交的
"""

# ╔═╡ 2ee1e794-9ddb-46e3-a8d1-73e4d9a0c762
begin
   function   inc_angle(vec1,vec2)
		vdot=dot(vec1,vec2)
	    l1,l2=length(vec1),length(vec2)
        angle=round(acos(vdot/(l1*l2))/pi,digits=2)
	    return L"%$(angle) \pi"

   end
	function   inc_angle2(vec1,vec2)
		vdot=dot(vec1,vec2)
	    l1,l2=length(vec1),length(vec2)
        angle=round(acos(vdot/(l1*l2))/pi,digits=2)
	    return angle
   ...
   function length(vec)
			length=size(vec)
			arr=[(vec[i])^2 for i in 1:length[1]]
			return sqrt(sum(arr))
   end
...
# ╔═╡ db0e831a-6262-428f-898d-6b65a2589b4d
let
	vec1,vec2=[2,2],[0,3]
	inc_angle(vec1,vec2)
end

# ╔═╡ cf87ecdb-7827-4bdd-b4d2-073127e3cb2d
let
    u, v = [2,1,-1] , [1,3,5]
	inc_angle(u,v)
end

# ╔═╡ e4dddc18-311d-4aac-a053-154041e510fb
begin
        store=Dict()
...
# ╔═╡ 7ebd3f0a-aeea-407e-851b-4f4793b220b1
md"""
## 投影

计算一个向量与另一个向量方向相同的组分有很多应用, 获取一个向量在另一个向量方向上的组分就定义为投影. 

定义二维空间向量 $u=\begin{bmatrix}1\\ 2 \end{bmatrix}$ ,$v=\begin{bmatrix}3\\1\end{bmatrix}$

如图

 $(store["proj"])

 向量 $\vec{oq}$ 在向量 $\vec{u}$ 方向上的投影表示为$\vec{op}$

  向量 $\vec{u}$ 在向量 $\vec{v}$ 之间的夹角定义为$\theta$
...
$cos \theta =\frac{|op|}{|oq|}=\frac{|op|}{||v||}$

所以有:

$|op|=||v||cos \theta$

通过点积公式, 我们知道$u,v$ 之间的关系为:

$u\cdot v=||u|| \ ||v||cos \theta$

化简后得到:

$||v||cos \theta=\frac{u\cdot v}{||u||}$

因此$|op|$ 表示为:

$|op|=\frac{u\cdot v}{||u||}$

上述计算结果定义为$\vec{v}$ 在$\vec{u}$ 方向的分量(不是向量)

 $\vec{v}$ 在$\vec{u}$ 方向的分量乘以标准化的 $\vec{u}$ 向量就得到$\vec{v}$ 在$\vec{u}$ 上的投影

$proj_u(v)=\vec{op} =\frac{|op|}{||u||}u=\frac{u\cdot v}{||u||^2}u$
$=\frac{u\cdot v}{u\cdot u}u$

图中的$p$ 点坐标就是根据这个公式计算得来

```julia
   projvonu(v,u)=(dot(u,v)/dot(u,u))*u
```
"""

# ╔═╡ 86ca1576-80d8-497d-ae29-b21d4bdc04f5
let
	zero,v,u=[0,0],[1,2],[3,1]
	proj=projvonu(v,u)
	ann=[
		(0.5,1.2,text(L"v",pointsize=16, color=:blue,rotation=45)),
		(2.5,0.6,text(L"u",pointsize=16, color=:blue,rotation=15)),
		(1.5,0.4,text(L"p",pointsize=12, color=:black,rotation=0)),
		(1.1,2.0,text(L"q",pointsize=12, color=:black,rotation=0)),
    ]
	p1=vec_plot(zero, u)
	p2=vec_plot!(zero, v)
	p3=scatter!([zero[1],u[1],proj[1]],[zero[2],u[2],proj[2]],ms=6,mc=:black,label=false)
	p4=plot!([v[1],proj[1]],[v[2],proj[2]],ls=:dash,lw=1,color=:black,label=false)
    p5=plot!(ann=ann)
	save("proj",p5)
   
end

# ╔═╡ 5f8890dc-520f-49ce-88b7-2edaf89b369f
md"""
## 分解向量

!!! definition

    假设一个非$0$ 向量$u$, 给定一个向量$v$, 只存在两个向量,满足如下关系:

     $v=v\parallel +v \perp$

     $v\parallel$ 与$u$ 有标量倍乘关系, $v \perp$ 正交于$u$

 $(store["decomposition"])

之前我们已经看过平行于$u$ 的向量.  垂直于$u$ 的向量为$v \perp=v-v\parallel$
"""

# ╔═╡ d4e5d446-5278-441a-a5bc-76d6c5d50f5e
let
	zero,v,u=[0,0],[1,2],[3,1]
	proj=projvonu(v,u)
	perp=v-proj
	ann=[
		(0.5,1.2,text(L"v",pointsize=16, color=:blue,rotation=45)),
		(2.5,0.6,text(L"u",pointsize=16, color=:blue,rotation=15)),
		(0.8,0.2,text(L"v \parallel ",pointsize=14, color=:green,rotation=15)),
		(-0.3,0.5,text(L"v \perp ",pointsize=14, color=:blue,rotation=15))
	...
	p2=vec_plot!(zero, v)
	p3=scatter!([zero[1],u[1],proj[1]],[zero[2],u[2],proj[2]],ms=6,mc=:black,label=false)
	p4=plot!([zero[1],proj[1]],[zero[2],proj[2]],ls=:dash,lw=3,color=:green,label=false,arrow=true)
	p5=plot!([zero[1],perp[1]],[zero[2],perp[2]],ls=:dash,lw=3,color=:blue,label=false,arrow=true)
    p6=plot!(ann=ann)
	save("decomposition",p6)
