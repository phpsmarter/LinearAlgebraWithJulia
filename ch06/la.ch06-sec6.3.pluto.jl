...
# ╔═╡ 529630fc-00fa-11ed-2bea-8bd8b08e7701
begin
	using CairoMakie, PlutoUI,HypertextLiteral,LaTeXStrings,Symbolics,LinearAlgebra,RowEchelon,Latexify
	
end

# ╔═╡ e2e6cc68-c02e-41f1-8d1e-91ea01e49eb8
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ 08de4028-f366-422f-9119-0bef2311b425
begin
	a,b,c,d,e,f,g,h,i,j=[0,0],[1,0],[1,2],[2,2],[2,3],[1,3],[1,4],[3,4],[3,5],[0,5]
	matrix1=hcat(a,b,c,d,e,f,g,h,i,j,a)
	m,n=size(matrix1)
	#z=[0 for x in 1:n]'
	z=fill(0,(n,))
	matrix3d=vcat(matrix1,z')
	latexify(matrix1)
end

# ╔═╡ 2aff8996-267d-4549-ab3d-8dd5f4ac376a
latexify(matrix3d)

# ╔═╡ 6b534c42-d854-4073-b05f-8e68e7e1ce57
let
...
xs1,ys1=matrix1[1,:],matrix1[2,:]
xs2,ys2 =matrix2[1,:],matrix2[2,:]
lines!(ax1,xs1, ys1,color = :red)
arrows!(ax1,[0,0], [0,0], [3,0], [0,6])
arrows!(ax1,[0,0], [0,0], [-3,0], [0,6])
lines!(ax1,xs2, ys2)
...
# ╔═╡ 7119aef3-b5e1-4d90-bcc1-b19e05679ed8
let
θ=0.4*pi
A=[cos(θ) -sin(θ) ; sin(θ) cos(θ)]
matrix2=A*matrix1
fig = Figure()
ax1 = Axis(fig[1, 1],aspect = 1,title = "origin",)
ax3 = Axis(fig[1, 2 ],aspect = 1,xgridvisible = false,
        ygridvisible = false,yticksvisible = false,xticksvisible = false, yticklabelsvisible = false,xticklabelsvisible = false)
ax2 = Axis(fig[1, 3],aspect = 1,title = "transform",)
...
text!(ax3,5, 5, text = L"rotation \Rightarrow ",textsize =42, color = :black,align =(:center,:center))
hidespines!(ax3)
fig

end

# ╔═╡ 2b5f2dd5-6edd-4eaf-bd67-26ae3fae7689
let
...
arrows!(ax1,[0,0], [0,0], [3,0], [0,6])
lines!(ax2,xs2, ys2)
#lines!(ax3,0.5..20, x -> sin(x) / sqrt(x), color = :black)

fig

end

# ╔═╡ f3318483-5654-4fef-88d9-29056b95d402
let
...
# ╔═╡ 05a836aa-de17-4544-b713-42c12b44fc02
let
...
# ╔═╡ aa54ced0-7118-4785-bccf-80e1c560e4c8
let
    A=[]
    push!(A,[1 0 ; 0 1] )
    push!(A,[0 1 ; 1 0])
    ...
    f = Figure()
    axes = [Axis(f[i, j]) for j in 1:2, i in 1:3]

   for (idx,ax) in enumerate(axes)
	       if idx==6
			   
		   else
			matrix=A[idx]*matrix1
			xs,ys =matrix[1,:],matrix[2,:]
	        lines!(ax,xs,ys,)
			limits!(ax, BBox(0, 10, 0, 10))
		   end
   end
   delete!(axes[2,3])
	f
...
# ╔═╡ efa5fc45-86b9-4de9-a393-51dba4f9eb23
let
	@variables θ
...
# ╔═╡ 0d93fc1e-2413-4483-be3d-1348b8969519
let
	θ=0.4*pi
	A=[cos(θ) -sin(θ) 0 ; sin(θ) cos(θ) 0; 0 0 1]
	matrix2=A*matrix3d
	fig = Figure()
	ax1 = Axis3(fig[1, 1],title = "origin",)
	ax2 = Axis3(fig[1, 2],title = "transform",)
	xs1,ys1,zs1=matrix3d[1,:],matrix3d[2,:],matrix3d[3,:]
	xs2,ys2,zs2 =matrix2[1,:],matrix2[2,:],matrix2[3,:]
	lines!(ax1,xs1, ys1,zs1,color = :red)
