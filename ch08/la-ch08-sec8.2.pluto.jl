...
# ╔═╡ 361c0bf4-0bc4-11ed-1031-b32efa5d21f5
begin
        using PlutoUI, Plots,DataFrames,HypertextLiteral,LaTeXStrings,Symbolics,LinearAlgebra,RowEchelon,Latexify,Polynomials
        gr()
        theme(:bright)
...
# ╔═╡ b06c4bfe-75e5-46b8-b070-e56e38c8aedb
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ 8c83d16d-b5e6-442f-b5e7-b943d93a6729
md"""
# ch07  sec7.2  求特征值

!!! outcomes

    - A. 求矩阵的特征多项式, 特征值和特征向量
    - B. 求三角矩阵的特征值
"""

# ╔═╡ f9e2002a-82df-478d-a2ee-2d3f822a6e66
md"""
!!! definition

    给定 $n\tiems n$ 矩阵 $A$,和标量$\lambda$,$\lambda$ 是 A的特征向量需要满足如下条件:

$det(A-\lambda I)=0$

   也就是矩阵$A-\lambda I$ 的行列式为$0$


直接看例子
"""

# ╔═╡ 7bb8e8ec-6f3c-49eb-858b-6641229c9f4f
latexify([-5 2;-7 4]) 

# ╔═╡ 4ff93183-f3ca-4922-bebc-346ab76c83a8
let
...
# ╔═╡ 5f935d30-afc6-4ee3-86af-31ec38c90b93
md"""
化简后求根得到 $λ=-3, 2$
"""

# ╔═╡ e5410854-b405-4990-a560-3be04f3b7194
md"""
!!! info
    可以通过Polynomial.jl 软件包提供的方法来构建多项式和求根

    点击右下方 📚 Live docs, 然后点击下面代码代码中的 `Polynomial`
"""

# ╔═╡ 48f692f3-9c7b-4860-b01a-f57711199b6d
let
...
# ╔═╡ e27af54b-5504-47ce-bb82-309656cf2892
md"""
!!! example
    example 2  求下面矩阵的特征值
"""

# ╔═╡ dae63b37-f8e8-4918-a7f9-8a81f35ba000
latexify([5 -4 4;2 -1 2 ; 0 0 2]) 

# ╔═╡ 2af9d56d-98ab-4562-98c8-5f67c440bf61
let
 @variables A,λ,I,x,poly
  A=[5 -4 4;2 -1 2 ; 0 0 2]
  I=diagm([1,1,1])
...
# ╔═╡ ca11306c-b793-4db4-b6c2-72882ae74ab6
md"""
化简后求根得到 $λ=1,2,3$
"""

# ╔═╡ c2dc1ef9-d768-4af2-a648-17696ab187c9
let
	# 列出多项式, 求根
	poly=Polynomial([6,-11,6,-1],:λ)
	roots=roots(poly)
end

# ╔═╡ a9b70eaa-ff18-4ddf-8cb4-7dfcc3a53a8a
md"""
!!! example
    example 3 求下面矩阵的特征值
"""

# ╔═╡ 273d27f9-c0c1-404e-8553-80300ac0107d
latexify([0 -1; 1 0]) 

# ╔═╡ 1faa058e-37ef-4419-9d0e-865e20e5f448
let
 @variables A,λ,I,x,poly
  A=[0 -1; 1 0]
  I=diagm([1,1])
...
# ╔═╡ ccda05f9-d342-4e1d-bf21-bb44e9b23e8e
md"""
这个方程没有实根, 但是可以得到复特征值
"""

# ╔═╡ 09a170ec-d2c6-47ea-bd59-6115752e575e
let
	# 列出多项式, 求根
	poly=Polynomial([1,0,1],:λ)
	roots=roots(poly)
end

# ╔═╡ 458b27d0-6a20-4533-a1a5-a574d6d22faf
md"""

复特征值的行为和实根不同, 这个后面会讲到

"""

# ╔═╡ 233abad7-c6c7-4132-85b9-80ebe8933540
md"""
上面三个例子中得到了三个多项式就成为特征多项式.

得到特征多项式后,可以获得齐次多项式的根, 然后可以求出各个特征值对应的特征向量

"""

# ╔═╡ 33b7f53f-f6ff-4fb6-8159-010c5c8e461a
md"""

!!! algorithm
    求特征值和特征向量

    1. 列出特征多项式:$det(A-\lambda I)$
    2. 求出特征多项式的根作为特征值.
    3. 对应每个特征值求出齐次方程组$(A-\lambda I)v=0$ 的解集,解集就是特征向量


!!! example 

    example 4 求下面矩阵的特征值和特征向量

"""

# ╔═╡ 0b86f45b-6c62-4e92-98ec-b54095ea6c33
latexify([3 0 2; 6 4 3; -4 0 -3]) 

# ╔═╡ bea15fdf-7a47-445e-963c-606866fecf1c
let
 @variables A,λ,I,x,poly
  A=[3 0 2; 6 4 3; -4 0 -3]
  I=diagm([1,1,1])  
...
# ╔═╡ 38e7142e-3c3b-436a-a006-ec522399176f
let
 @variables A,λ,I,x,poly
  A=[3 0 2; 6 4 3; -4 0 -3]
  I=diagm([1,1,1])
  poly=det(A-λ*I)
  res=simplify(expand(poly))
  #latexify(res~0)
  
  
end 

# ╔═╡ e3ffcfaa-319b-4a86-b6f0-5e1d6633d0ad
let
	# 列出多项式, 求根
	poly=Polynomial([-4,1,4,-1],:λ)
	roots=roots(poly)
end

# ╔═╡ 370d0407-fa4e-4269-a7df-deeb5a93fbeb
let
	# 分别列出三个特征值的齐次线性方程组
	@variables A,λ,I,v,poly,m
     A=[3 0 2; 6 4 3; -4 0 -3]
     I=diagm([1,1,1])
	
	 m=hcat(A-(-1*I),[0 ,0, 0])
	 latexify( rref(m))
	 # 通过增广矩阵可以看到, 有一个自由变量, 1倍的第一列, 0 倍的第二列, 2倍的第三列组合可以得$0$向量, 所以 解集的基为:
end

# ╔═╡ c37ee84b-384b-4588-bcdb-ad29784dff9f
latexify([1 ,0, 2]) 

# ╔═╡ 6311a732-0dab-442e-92b6-51794b43d0f3
let
	@variables A,λ,I,v,poly,m
     A=[3 0 2; 6 4 3; -4 0 -3]
     I=diagm([1,1,1])
	 ...
	 m=hcat(A-(1*I),[0 ,0, 0])
	 latexify( rref(m))
	# 通过增广矩阵可以看到, 有一个自由变量, -1倍的第一列, 1倍的第二列, 1倍的第三列组合可以得$0$向量, 所以 解集的基为:
end

# ╔═╡ 846d48d0-b494-4894-9165-3d41f09c3c22
latexify([-1 ,1, 1]) 

# ╔═╡ 02a1d945-5e5c-4aa4-a226-e8709ab4075c
let
	@variables A,λ,I,v,poly,m
     A=[3 0 2; 6 4 3; -4 0 -3]
     I=diagm([1,1,1])
	 ...
	 m=hcat(A-(4*I),[0 ,0, 0])
	latexify( rref(m))
	# 通过增广矩阵可以看到, 有一个自由变量, 0倍的第一列, 1倍的第二列, 0倍的第三列组合可以得$0$向量, 所以 解集的基为:
end

# ╔═╡ 59571ce6-635d-4704-863f-f1e8f202d30e
latexify([0 ,1, 0]) 

# ╔═╡ f17e3997-4bbd-4a7a-a731-1988fc7933d2
md"""
!!! example 

    example 5 求下面矩阵的特征值和特征向量
"""

# ╔═╡ 65c2943c-8e2d-4a77-a74c-ff8fc8c7a61b
latexify([2 2 -2; 1 3 -1; -1 1 1]) 

# ╔═╡ fcb80ada-6394-433a-8cef-8e55670bf2e7
let
	# 首先列特征多项式
  @variables A,λ,I,x,poly
  A=[2 2 -2; 1 3 -1; -1 1 1]
...
# ╔═╡ 48b0af03-a0e0-4b34-9455-f560ee8361a6
let
	
    # 列出多项式, 求特征值
	poly=Polynomial([0,-8,6,-1],:λ)
	
	roots=roots(poly)
	
end

# ╔═╡ ea5ab171-d4a9-464d-9ecb-3dd21466b386
let
	@variables A,λ,I,v,poly,m
     A=[2 2 -2; 1 3 -1; -1 1 1]
     I=diagm([1,1,1])
	 ...
	 m=hcat(A-(0*I),[0 ,0, 0])
	 latexify( rref(m))
	# 通过增广矩阵可以看到, 有一个自由变量, 1倍的第一列, 0倍的第二列, 1倍的第三列组合可以得$0$向量, 所以 解集的基为:
end

# ╔═╡ 2731fbf0-21c8-4166-b9f2-4a4a72ad108f
latexify([1 ,0, 1]) 

# ╔═╡ dde0dc93-f388-4060-be0a-7537decf563e
let
	@variables A,λ,I,v,poly,m
     A=[2 2 -2; 1 3 -1; -1 1 1]
     I=diagm([1,1,1])
	 ...
	 m=hcat(A-(2*I),[0 ,0, 0])
	 rref(m)
	# 通过增广矩阵可以看到, 有一个自由变量, 0倍的第一列, 1倍的第二列, 1倍的第三列组合可以得$0$向量, 所以 解集的基为:
end

# ╔═╡ 69485ab8-b22a-454b-9f96-b2fd67c7525a
latexify([0 ,1, 1]) 

# ╔═╡ c84bb094-81b2-4935-a794-7fc1d92ab840
let
	@variables A,λ,I,v,poly,m
     A=[2 2 -2; 1 3 -1; -1 1 1]
     I=diagm([1,1,1])
	 ...
	 m=hcat(A-(4*I),[0 ,0, 0])
	 latexify( rref(m))
	# 通过增广矩阵可以看到, 有一个自由变量, 1倍的第一列, 1倍的第二列, 0倍的第三列组合可以得$0$向量, 所以 解集的基为:
end

# ╔═╡ 18bd720f-b676-44a7-a5fd-8ba6a93d6359
latexify([1 ,1, 0]) 

# ╔═╡ 0727afaa-f409-4c64-8688-42f44e91c3d8
md"""
!!! example 
...
# ╔═╡ bf5d4e6c-b2e5-4494-bcda-0a72f31f47ba
latexify([1 2 4; 0 4 7; 0 0 6]) 

# ╔═╡ 9c3f60d6-75af-4818-bf24-dd1003c378e1
let
	# 首先列特征多项式
  @variables A,λ,I,x,poly
  A=[1 2 4; 0 4 7; 0 0 6]
  ...
  # 可以看到特征值直接可以求出, 就是三角矩阵对角线线的值
	
end

# ╔═╡ bc8f733b-dbbd-4cb1-b486-83ee7097dd4e
