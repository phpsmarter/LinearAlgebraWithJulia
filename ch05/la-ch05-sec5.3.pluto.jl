...
# ╔═╡ ae05578c-fff2-11ec-169a-dd87db7d3632
begin
...
# ╔═╡ 82f92212-dc9d-473e-977d-f9323199ed3e
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ a2a1f555-e7ac-489f-b1ef-90220927e7cc
md"""
# ch05 sec5.3   Rⁿ 空间的子空间

!!! outcomes

    - A. 判断一个集合是否是$R^n$ 空间的子空间
    - B. 认出生成的$R^n$ 子空间
    - C. 
"""

# ╔═╡ c15c1e86-0708-42b1-9d2b-2f598889c636
md"""
在数学中一条重要原则就是遵守规则

任何标量乘以$0$ 向量都可以表示表示为$0$ 向量, 所以可以根据规则$0$ 向量扩张称为$0$ 空间

$0$ 向量是比较特殊的向量, 任何向量的组合至少都可以得到$0$ 向量. 


一维向量乘以一个实数标量,可以扩张称为一条直线.  

两个向量可以扩张成为一个平面. 

向量线性组合只能改变向量中组分的量值, 不会改变向量的维度, 这个称为封闭性
...
!!! definition
    ...
    子空间(subspace)
     ...
     一个$R^n$ 空间的向量 $V$ 子集称为子空间,如果满足一下条件:

    1.  $V$ 包含$R^n$ 空间的 $0$ 向量
    2.  $V$ 是加法封闭的, 对于所有属于$V$ 的向量, 例如$u,v \in V$,则他们的和 $u+v \in V$ 
    3.  $V$ 标量乘法也是封闭的, 对于属于$V$ 的向量,例如$w\in V$,和标量$k$, 则$kw$ 仍然是$V$ 的子集

    
"""

# ╔═╡ 816df46f-caf3-4ff6-803a-bdcf72aa5178
md"""
!!! props
    齐次线性方程组的解空间

    假定齐次线性方程组$Ax=0$ , 其中$A$ 是$m\times n$ 的矩阵, 那么方程组解的集合:

    $V=\left \{ x \in R^n \ | \  Ax=0 \right \}$

    是$R^n$ 的子空间. 称为方程组的解空间
"""

# ╔═╡ 0d39c6b7-6a85-425b-81f9-7b045b0b8ed5
md"""

!!! example 
	example 6

    说明 平面$2x+3y-z=0$ 是$R^3$ 的子空间



因为 $2x+3y-z=0$  是齐次方程,它的解空间为:

$\begin{Bmatrix}
\begin{bmatrix} x\\y \\z \end{bmatrix} &   | & 2x+3y-z=0&
\end{Bmatrix}$

所以根据性质可知$\begin{bmatrix} x\\y \\z \end{bmatrix}$ 是$R^3$ 的子空间

"""

# ╔═╡ e48429a0-2b81-4b49-a37d-9a3244368c9c
md"""
...
!!! example
    ...
    - a. 直线
    $\begin{bmatrix} x\\y \\z \end{bmatrix}=\begin{bmatrix} 1\\2\\2 \end{bmatrix}+t\begin{bmatrix} 1\\0 \\1 \end{bmatrix}$
    ...
    -  c. 向量集合
    $\begin{Bmatrix}\begin{bmatrix} x\\y \\z \end{bmatrix} &| & x,y,z \ge 0&\end{Bmatrix}$


上面三个例子都不是子空间, a, b 不包含$0$ 向量, c 包含 0 向量, 加法也是封闭的, 但是标量乘法,标量值如果取负数, 向量就没有包含在空间中, 所以也不是子空间
"""

# ╔═╡ c8e03e7a-66b0-47af-8c92-3c65e349e26e


# ╔═╡ b79af832-d72b-4109-8a7b-3dadc939d7e5
begin
...
# ╔═╡ a0bf9e2e-d3e8-400b-bf35-f6b9cc79bbfd
let
	l = @layout [
    a{0.2w} b{0.2w} c{0.6w} ]
	...
	p5=scatter([0],[0],ms=4,mc=:red,ticks=false,lims=(-6,6),label=false)
	p1=scatter(xs,ys,label=false,markersize=3,framestyle=:origin,color=:red)
	p2=plot!([0,2*v1[1]],[0,2*v1[2]],arrow=3,lw=3,label=false)
	p3=plot!([0,2*v2[1]],[0,2*v2[2]],arrow=3,lw=3,;label=false)
	p4=plot!(p1,p2,p3,link=:all)
	

	p6=vec_plot(zero,point)
    p7=plot!(line.(range), range, label=false,ticks=false)
	p8=plot!(p6,p7)
	plot!(p5,p8,p4,layout=l)
end

# ╔═╡ 0c645c92-a5cb-4149-9d5a-f4d1d92291ca


# ╔═╡ e1cf1798-249a-4ece-b533-6cca549ab0b1


# ╔═╡ 3413de18-8578-4acc-8478-90b98cbeefc8


# ╔═╡ 299b55ef-436a-4a34-80e4-a83047c0553b


# ╔═╡ 776d6185-3b8b-43bf-85e1-adcb78cd6000
