...
# ╔═╡ c13ac870-f915-11ec-2ff2-3743cebabb88
begin
...
解线性方程组的策略是通过连续的变换把方程组变为简单形式. 执行变换操作时方程组的解没有改变,也就是多次变换没有改变方程组的自身的状态, 只是形式发生改变
...
# ╔═╡ ad5928b0-1aed-4e50-bccf-aba89202aaae
md"""
!!! theorem 
    基本操作和方程解

    从最简单的形式开始. 如果方程组有两个方程:
    ...
    2. 第二行 方程等式两边同时乘以一个系数$k(k \neq 0)$:
    $E_1=b_1$
    $kE_2=kb_2$

    3. 把 2 步得到的方程加到另一个方程两边,$k也可以等于1,也可以等于 0$

    $E_1=b_1$
    $E_2+kE_1=b_2+kb_1$
"""

# ╔═╡ edd06cf8-2f77-4eaa-8ed3-4bf9e533b384
md"""
!!! example
    example 1 
...
倍乘,行相加操作用了symbolic.jl 软件包的方法, 参见后面的代码
"""

# ╔═╡ b580d80f-0fee-446c-8edd-5f0afb17e173
md"""
将 $z=3$ 带入❹式 可以求得$y$:
...
# ╔═╡ 024283f8-9260-4d4d-b06d-0e995c115770
md"""
将 $z=3,y=2$ 带入❶式 可以求得$x$:
...
# ╔═╡ d3c3c90c-cc63-4ae9-a397-bd47335beba0
md"""
方程组的解为: 
...
# ╔═╡ d85e95c3-930c-466e-866f-5a8d4c277250
md"""
在线性方程组基本操作中, 可以看到直到最后的回带操作之前, 变量都没有参与实际操作, 变换都是系数再变换,因此更为简介的方式是暂时把变量放到一边, 只考虑系数, 线性方程组就变形为一个系数数字组成的序列
下面的 形式增加了常数项$b$, 称为增广矩阵.

增广矩阵由两部分组成, 用竖线分割, 左边的是系数矩阵, 右边的是常数项矩阵. 
...
代表三个方程里变量为$x$ 的系数项组成的列

这些内容如果是首次接触会感到很陌生,需要一点时间熟悉



"""

# ╔═╡ 100ce119-9d83-445c-a3f8-5124425ece66
md"""
!!! definitions
    **增广矩阵定义**
...
# ╔═╡ 8ae957b3-ff75-4377-a44a-16d3a494d983
md"""
 增广矩阵表示为:
"""

# ╔═╡ bb307bd4-2d4d-43bf-8c12-45fbf79c0b13
md"""
新的第二行为原有第二行减去$2$ 倍的第一行
"""

# ╔═╡ cffbacab-5dc8-4299-85df-5a090ca25609
md"新的第三行等于原有第三行减去$2$ 倍的新第二行"

# ╔═╡ 8fe5551b-7b67-4719-8312-203faa1f70be
md"""
还可以继续进行变形为:
"""

# ╔═╡ cbc12b57-9501-400a-8e00-fd91a3985edc
md"""
现在的增广矩阵的系数矩阵是一个对角线为$1$,其他位置为$0$ 的矩阵,称为单位矩阵, 常数项矩阵就是方程组的解
"""

# ╔═╡ c4cb3943-d12a-4e0d-8831-f392d87ae8e3
begin
	function scalar(scale,r,b)
	   if scale==0  return r,b
	   else
         return scale*r,scale*b
	   end
	   
    end

	function  add(r1,b1,r2,b2)
...
# ╔═╡ fde0f6a1-1fc8-49c3-851a-e52ac6fa6a8a
md"""
 行❶两边同时乘以$-2$ 得到:
...
# ╔═╡ cbbddd3b-bc1e-4839-b443-e57ddb2531e1
md"""
新的一行左右两边分别加到❷两边,得到❹ 式

 $(store["nr2"])= $(store["nb2"])    ❹ 
"""

# ╔═╡ 595d7e3f-8bb4-476b-bd00-73e10aa79029
md"""
上式两边同时乘以$-2$,等式两边分别加到$❸$ 式两边,得到❺

 $(store["nr3"])= $(store["nb3"]) ❺ 

"""

# ╔═╡ 57005f80-da65-4c83-9723-9ab0aac38104
md"""
...
# ╔═╡ fcdf8c49-db75-405a-b8d4-d9e73c2dc646
begin
	@variables x y z
	r1,r2,r3=x+3y+6z,2x+7y+14z,2y+5z
	b1,b2,b3=25,58,19
	save("r1",r1)
	save("b1",b1)
    sr1,sb1=scalar(-2,r1,b1)
	@show sr1==sb1
	save("sr1",sr1)
	save("sb1",sb1)
    nr2,nb2=add(r2,b2,sr1,sb1)
	save("nr2",nr2)
	save("nb2",nb2)

	snr2,snb2=scalar(-2,nr2,nb2)
	nr3,nb3=add(r3,b3,snr2,snb2)
...
# ╔═╡ 3783ce67-c686-458d-8ebb-ba99db907f41
let
	R1=[1 3 6 25]
	R2=[2 7 14 58]
	R3=[0 2 5 19 ]
...
# ╔═╡ 15950fd2-aad0-43bb-9ad6-eec59fc7d11c
store["m1"]

# ╔═╡ 202f872b-39a7-4d91-9114-72c9abac74f9
store["m2"]

# ╔═╡ b059a48d-13cd-4f65-9d22-218f45d569c3
store["m3"]

# ╔═╡ c387a295-d19c-4311-a389-c09e9768cd16
md"""
变换得到的新矩阵就是线性方程组变换后的形式
...
# ╔═╡ 03cb7c1a-a343-42d0-8850-674778eb3605
let
	matrix3=read("m3")
    R1,R2,R3=matrix3[1,:],matrix3[2,:],matrix3[3,:]
	R1=R1-6*R3
	R2=R2-2*R3
	R1=R1-3*R2
	matrix=[R1';R2';R3']
