...
# ╔═╡ e0a27054-7b40-4efe-be5f-f6afc3edf72f
begin
...
# ╔═╡ 931d1723-44e7-42ea-b2b2-fb5a7735cd2a
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ 6dfca12b-122c-41eb-ae1d-3e669db8e06e
md"""
# ch06 sec6.1 线性变换

!!! outcomes

    - A 判断一个向量函数:$T:R^n \rightarrow R^m$  是否是线性变换
"""

# ╔═╡ 0454ea36-59c3-426f-bfa9-11e65f6b5479
md"""
前面我们已经在表明这个概念: $Ax=b$ 的线性方程组符合函数的三要素, 定义域, 值域和一套规则. 

当函数的拓展到向量领域, 原理和微积分中的函数完全一样. 

看下面的例子

$T_1=\left ( \begin{bmatrix}x\\y \end{bmatrix} \right )=\begin{bmatrix} x^2\\x+y\\y^2 \end{bmatrix}$


$T_2=\left ( \begin{bmatrix}x\\y\\z \end{bmatrix} \right )=\begin{bmatrix} x+y\\x+y+z\\0 \end{bmatrix}$

$T_3=\left ( \begin{bmatrix}x\\y \end{bmatrix} \right )=\begin{bmatrix} e^{x+z}\\ \sqrt{y} \end{bmatrix}$


我们可以构造出函数
"""

# ╔═╡ 0f5d326f-9dd5-43a3-970d-c8645a6209b3
begin
	function t1(vec)
	  x,y=vec[1], vec[2]
	  return   [x^2, x+y ,y^2]
...
# ╔═╡ d1ac315d-0b93-4844-b4a9-2f40c73f208b
latexify(t1([1 ,2]))

# ╔═╡ 37ba988b-d7ed-499b-a2c5-60de6db9dbbd
latexify(t1([0 ,1]))

# ╔═╡ a6f64acd-7b7d-4031-a95d-dd55306f4684
latexify(t2([1,2,3]))

# ╔═╡ 8c6aebcc-d108-4b7d-8fa9-ce6320c7ebd6
latexify(t2([1,2,10]))

# ╔═╡ e93c4a9d-0dc8-45d9-aaf6-cb8c82d7ba06
md"""

$T_1:R^2 \rightarrow R^3$
$T_2:R^3 \rightarrow R^3$
$T_3:R^2 \rightarrow C^2$

稍后要理解的是,在$T_1$ 规则里 二维向量可以映射为三维向量,  

在$T_2$ 映射中, 尽管获得的所有向量第三个分量都为$0$, 但结果仍然是三维向量,与二维向量是不同的. 
 
 $T_3$ 有可能会没有实数解

"""

# ╔═╡ 28358b8a-1e83-4536-b713-7c566ed670f7
md"""

!!! defintion

    线性变换(linear transformation) 概念

    一个向量函数:$T:R^n \rightarrow R^m$ 如果要成为线性变换必须满足下面两个条件:

    1.  $T$ 保留加法性质, 对于向量:$v,w \in R^n$, 有 $T(v+w)=T(v)+T(w)$ 
    2.  $T$ 保留标量乘法性质, 对于向量 $v \in R^n$ 和标量 $k \in R$, 有$T(kv)=kT(v)$
"""

# ╔═╡ 94d3435e-6b03-4f05-927d-e308fea9048c
md"""
!!! example

    example 1 上面定义的$T_1,T_2,T_3$ 函数是不是线性映射?
"""

# ╔═╡ b3551f64-6992-4738-bd98-164639228de0
begin
  vec1=[1,2,3]
  vec2=[3, 4,5]
  latexify([2*t1(vec1) t1(2*vec1)])   #  T1(2v) 不等于 2T1(v)
end

# ╔═╡ b328f321-38a2-44cb-94c3-d7910ce409fa
latexify([t1(vec1+vec2) t1(vec1)+t1(vec2)])  # T1(v+w) 也不等于 T1(v)+T1(w)

# ╔═╡ 9482feb5-d78c-4e76-922e-7be27ba372d0
md" 所以 $T_1$ 不符合线性映射要求"

# ╔═╡ 4fc197ea-456b-467f-a1a6-e66babf90988
begin
  @variables x_1, y_1,z_1,x_2,y_2,z_2
  v,w=[x_1,y_1,z_1],[x_2,y_2,z_2]
  latexify([t2(v+w) t2(v)+t2(w)])  #t(v+w)=t(v)+t(w)
end

# ╔═╡ 6edce0c2-0d84-4c10-b2d9-ce58c6b5da90
latexify([2*t2(v) t2(2*v) ])   # t(2v)=2t(v)

# ╔═╡ 6a3fb5b8-cdd3-49e7-b7e9-538c6a23f373
md" 从上面代码可以看到$T_2$ 符合线性映射规则"

# ╔═╡ 3e33ba1d-e1c9-49d3-b7c2-6b64fbd6bf4f
begin
	a,b=[0,1,0],[1,1,0]
    latexify([t3(a+b) t3(a)+t3(b)])  #t(v+w) 不等于 t(v)+t(w) 
end

# ╔═╡ bff982a9-ede1-43de-a51a-84d9d9a09da7
latexify([t3(2*a) 2*t3(a)])   # t(2v) 不等于 2t(v)

# ╔═╡ 230142e3-1447-4727-b2b0-2586270e7faa
md" 从上面代码可以看到$T_3$ 不符合线性映射规则"

# ╔═╡ 4ddb5d1e-2f09-45b3-b972-bb91730bc3b7
