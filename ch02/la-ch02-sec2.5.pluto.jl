...
# ╔═╡ a21831be-fbfc-11ec-2138-75ce10c95545
begin
        using PlutoUI, Plots,DataFrames,HypertextLiteral,LaTeXStrings,Symbolics,LinearAlgebra,RowEchelon,CairoMakie
...
# ╔═╡ 4e38f477-0f07-4250-a52b-033cd083e9f3
md"""
...
# ╔═╡ 1181aed3-723d-47c2-81fa-bdb65ff71b59
md"""
...
# ╔═╡ 05a10f6c-92b5-41e5-9b3b-21c2d8b719ce
md"""

!!! example 
    example 4

    $R^4$ 空间中的两点$p=(1,2,-4,6)$ 和$Q=(2,3,-1,0)$ 之间的距离为:

    $d(P,Q)=\sqrt{(1-2)^2+(2-3)^2+(-4-(-1))^2+(6-0)^2}=\sqrt{47}$
"""

# ╔═╡ 0a4008f8-ef40-431b-a300-5b485a371209
md"""
##  向量的长度

!!! definition

    在$R^n$ 空间中的坐标向量u:$\begin{bmatrix} u_1\\ \vdots \\u_n \end{bmatrix}$ 的长度定义为: $||u||$,由公式:
...
# ╔═╡ 21789382-6a0d-47cb-80a9-4e606a67179e
begin
        store=Dict()
...
# ╔═╡ 1c7383ec-36d5-41c1-91ba-09cfa8bd86b7
md"""
## 二维空间中两个点之间的距离
...
# ╔═╡ 6810f60c-c63e-429b-a31c-00c73c4b2172
let
    ...
    p1=scatter!([P[1],Q[1]],[P[2],Q[2]],mc=:blue,label=false,xlims=(0,5),ylims=(0,5),frame=:semi)
...
# ╔═╡ 07337bde-250e-4a32-a464-12e381a0b2ab
md"""
## 三维空间中两个点之间的距离
...
# ╔═╡ fc6b9684-7f96-4868-82f8-4ce634c1734d
let
a,g=[0,0,0],[1,1,1]
dist(a,g)
end

# ╔═╡ 97c3538f-d7f9-410d-9aa4-0e3383d5fbc2
md"""
...
# ╔═╡ d0145463-0180-4965-8a6b-0c92037c7554
let
	plotly()
	P,Q=[1,2,3],[0,1,2]
...
# ╔═╡ a16f700f-b80a-4894-9727-81be890e4548
begin
	yshift=0.1
	...
	lab,lae,lbf,lcg=[a;b],[a;e],[b;f],[c;g]
	ldh,lag,lad,lbh,lac=[d;h],[a;g],[a;d],[b;h],[a;c]
	
  
	function makeCubic()
	 figure = Figure()
    ax = Axis3(figure[1, 1], aspect = :data, perspectiveness =0.3, elevation = 0.1*π,azimuth=-0.3*π, viewmode=:fit)
	hidedecorations!(ax)
	#ax.protrusions = (0, 0, 0, 20)
	
	lines!(ax,lowerplane[:,1],lowerplane[:,2],lowerplane[:,3],linewidth=1,color=:green)
	lines!(ax,upperplane[:,1],upperplane[:,2],upperplane[:,3],linewidth=1,color=:green)
	lines!(ax,lae[:,1],lae[:,2],lae[:,3],linewidth=1,color=:green)
	lines!(ax,lbf[:,1],lbf[:,2],lbf[:,3],linewidth=1,color=:green)
	...
	#lines!(ax,lag[:,1],lag[:,2],lag[:,3],color=:red,linestyle=:dash,linewidth=1)
	 # arrows!([Point3f(0,0,0)],[Vec3f(0.97,0.97,0.97)],arrowsize = 0.04,arrowcolor=:red,linecolor=:red,linestyle=:dot)  #定义矢量的方法

	text!("a:(0,0,0)", position =(0,0+yshift,0), align = (:left, :baseline),textsize=12,color=:red)
	text!("b:(1,0,0)", position = (1,0+yshift,0), align = (:left, :baseline),textsize=12,color=:red)
	text!("c:(1,1,0)", position =(1,1+yshift,0), align = (:left, :center),textsize=12,color=:red)
	text!("d:(0,1,0)", position =(0,1+yshift,0), align = (:left, :center),textsize=12,color=:red)

	text!("e:(0,0,1)", position = (0,0+yshift,1), align = (:left, :baseline),textsize=12,color=:red)
	text!("f:(1,0,1)", position = (1,0+yshift,1), align = (:left, :baseline),textsize=12,color=:red)
	text!("g:(1,1,1)", position = (1,1+yshift,1), align = (:left, :baseline),textsize=12,color=:red)
	text!("h:(0,1,1)", position = (0,1+yshift,1), align = (:left, :baseline),textsize=12,color=:red)
...
# ╔═╡ 96877fe1-5e83-4830-a730-ea3e8c7f2595
let
    fig=makeCubic() 
	figure=fig[1]
	ax=fig[2]
